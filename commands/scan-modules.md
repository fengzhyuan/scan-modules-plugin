...
3. Record git HEAD hash in INDEX.md for freshness tracking
4. **Auto-configure CLAUDE.md** (FULL_SCAN only):
   - Read the project's `CLAUDE.md` (if it exists)
   - Check if it already contains `scan-modules` or `modules/INDEX.md` — if yes, skip (idempotent)
   - If not, **append** the following snippet to `CLAUDE.md`:

```markdown

## Module Cache (auto-generated by scan-modules)

At session start, read `.claude/modules/INDEX.md` for project structure overview.
When searching or exploring the codebase, check module cache files (`.claude/modules/{module}.md`) for types and function signatures BEFORE using grep/glob on source files.
Run `/scan-modules:scan-modules update` if cache becomes stale.
```

   - If `CLAUDE.md` does not exist, create it with just this snippet
   - Print a message: `"Added module cache instructions to CLAUDE.md for auto-loading in future sessions."`
   - Do NOT modify `CLAUDE.local.md` — only touch `CLAUDE.md`
